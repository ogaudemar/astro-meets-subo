...
- The **derived** rows come from the English-only blog collection. The linked post stays
  English, but the row's title + summary on `/fr/changelog` are translated via
  `src/data/changelog-blog-fr.ts` (keyed by post slug), falling back to the English
  title/description when a slug is missing. A new locale would add a parallel `blogFr`-style map.
