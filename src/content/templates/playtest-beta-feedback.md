...
description: "Four one-tap ratings plus two open questions. Get a comparable satisfaction score and the 'why' in players' own words after every playtest or beta."
...
featureCallouts:
  - emoji: "⭐"
    ...
    desc: "Fun, engagement, polish and would-recommend, each answered with a single tap on a face emoji. No typing, so more players actually finish."
  - emoji: "📊"
    title: "A score you can compare"
    desc: "Each scale is weighted one to five, so the ratings roll up into one satisfaction score per session. Watch it move build over build instead of eyeballing raw answers."
  - emoji: "💬"
...
steps:
  ...
  - title: "Tune the scales"
    desc: "Each rating option is weighted one to five. Leave the defaults for a standard satisfaction score, or reweight if a dimension matters more."
...
faq:
  - q: "Is this an NPS survey?"
    a: "It's NPS-style. Instead of a single zero-to-ten score it uses four short rating scales that roll up into one satisfaction number, plus open follow-ups. Swap in a classic 'how likely are you to recommend' question if you specifically want NPS."
